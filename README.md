...
graph TD
    %% ==================== 视图层 ====================
    subgraph ViewLayer["1. 视图层"]
        direction LR
        QMCanvasView["QMCanvasView<br/>顶层容器"]
        View["View<br/>QScrollArea"]
        Viewport["Viewport<br/>实际渲染部件"]
    end
    
    %% ==================== 场景层 ====================
    subgraph SceneLayer["2. 场景层"]
        QMCanvasScene["QMCanvasScene<br/>场景管理&协调中心"]
    end
    
    %% ==================== 管理层 ====================
    subgraph ManagementLayer["3. 管理层"]
        direction LR
        Location["Location<br/>坐标变换管理"]
        LayerManager["LayerManager<br/>多图层合成"]
        Layer["Layer<br/>单图层容器"]
    end
    
    %% ==================== 绘制层 ====================
    subgraph DrawLayer["4. 绘制层"]
        direction TB
        QMDrawObject["QMDrawObject<br/>抽象基类"]
        QMDrawPen["QMDrawPen<br/>手绘线条"]
        QMDrawRect["QMDrawRect<br/>矩形绘制"]
        QMDrawPixmap["QMDrawPixmap<br/>位图渲染"]
    end
    
    %% ==================== 层次依赖关系 ====================
    ViewLayer --> SceneLayer
    SceneLayer --> ManagementLayer
    ManagementLayer --> DrawLayer
